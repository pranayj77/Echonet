...
class PlaxHypertrophyInferenceEngine:
        ...
        for si in tqdm(range(0, len(frame_map), batch_size)) if verbose else range(0, len(frame_map), batch_size):
            ...
            for p in batch_paths:
                if p not in clips:
                    ...
                    clips[p] = (c, np.zeros((len(c), h, w, channels_out), dtype=np.float64))
    ...
    def run_model_np(self, x: np.ndarray) -> np.ndarray:
        ...
        if self.model is None:
            self.load_model()
        input_tensor = torch.tensor(np.moveaxis(x, -1, 1) / 255,dtype = torch.float,device = self.device, requires_grad = False)
        with torch.no_grad():
...
if __name__ == '__main__':
    ...
    args = {
        ...
        'batch_size': (16, 'Number of frames to run inference on at once.'),
        'model_path': (model_paths['plax'], f'Path to model weights.'),
        'n_threads': (2, 'Number of threads to use while generating animations and results.'),
        ...
        'save_npy': (True, 'Save raw model predictions in numpy format to .npy file.'),
    }
